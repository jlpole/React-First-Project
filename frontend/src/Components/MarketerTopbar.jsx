...
export default function MarketerTopBar({user}) {
  const location = useLocation();
 

   const getPageTitle = () => {
    switch(location.pathname) {
      case '/Marketer/Business':
        return 'Business Details';
      case '/Marketer/Dashboard':
        return 'DashboardPage';
         case '/Marketer/PotentialBuyer':
        return 'Potential Buyers';
      case '/Marketer/Profile':
        return 'Profile';
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-3 pl-3 border-l border-gray-200">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-semibold text-gray-800">  {user?.name || 'Owner'}</p>
            <p className="text-xs text-gray-500">{user?.role || 'Undefined'}</p>
          </div>
