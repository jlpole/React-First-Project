...
import { toast } from 'react-toastify'; // or whatever toast library nimo

function OwnerMainContent({ user }) {
  const location = useLocation();

  useEffect(() => {
    if (location.state?.showWelcomeToast) {
      toast.success(`Welcome back, ${location.state.userName || user?.name}! 👋`, {
        position: "top-right",
        autoClose: 4000,
      ...
      // Clear the state para dili mag-refire on refresh
...
export default OwnerMainContent;